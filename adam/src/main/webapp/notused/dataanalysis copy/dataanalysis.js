(function() {
  ...
  app.service('DAService', function() {
    ...
    var dummyAllData = [
      /*{
      "labelName": "Drug1",
      "labelData": generateSequence(1, 23, 1)
    }, {
      "labelName": "Drug2",
      "labelData": generateSequence(2, 46, 2)
    }, {
      "labelName": "Drug3",
      "labelData": generateSequence(3, 69, 3)
    }, {
      "labelName": "CellType1",
      "labelData": generateSequence(1, 23, 1)
    }, {
      ...
      "labelData": [2, 2, 3, 4, 6, 8, 0, 0, 6, 5, 1, 1, 2, 4, 6, 7, 8, 8, 1, 4, 6, 9, 0]
    },*/
    ];
  ...
  app.controller('DataAnalysisController', ['$scope', '$routeParams', 'DAService',
    function($scope, $routeParams, DAService) {
      ...
      graphIt(DAService.plotData);/*.updateOptions({
  ...
  var appProjects = angular.module('projects', ['smart-table', 'mgcrea.ngStrap']);

  app.service('ProjectManagement', ['$rootScope',
    function($rootScope) {
      var service = {
        projects: [{
          "title": "Project b",
        ...
        getMeasurements: function() 
        {
          var well;
          var measurementData = [];
          for (var i = 0, iLen = ProjectManagement.currentWellCollection; i < iLen; i++) 
          {
              well = ProjectManagement.currentWellCollection[i];
  ...
  app.controller('gridsController', ['$scope', 'ProjectManagement', "deleteProject", "activeProject", "setActiveProject", "$timeout",
    function($scope, ProjectManagement, deleteProject, activeProject, setActiveProject, $timeout) {
      ...
      $scope.makeActive = function(proj) {
        ...
        ProjectManagement.makeActive(proj.title);
      };

      $scope.ActiveProject = activeProject.activeId;
      $scope.filterowner = '';

      $scope.projects = ProjectManagement.projects;
      ...
      $scope.updateWells = function() {
        $scope.wellCollection = [];
        for (var i = 0, iLen = ProjectManagement.projects.length; i < iLen; i++) {
          var project = ProjectManagement.projects[i];
          if (project.selected == true) {
            var wells = ProjectManagement.getWells(project.title);
            var wellLabel = "";
            ...
            for (var w = 0, wLen = wells.length; w < wLen; w++) 
            {
              ...
              if (ProjectManagement.labelFilter(wellLabel)
                && ProjectManagement.compoundFilter(wellCompound) 
                && ProjectManagement.substrateFilter(wellSubstrate))
        ...
        ProjectManagement.currentWellCollection = $scope.wellCollectionDisplay;
      };
      
      $scope.updateLabels = function() {
        selectedLabels = ProjectManagement.getOnlySelectedLabels();
        ProjectManagement.labels = [];
        for (var i = 0, iLen = ProjectManagement.projects.length; i < iLen; i++) {
          var project = ProjectManagement.projects[i];
          if (project.selected == true) {
            var wells = ProjectManagement.getWells(project.title);
            for (var w = 0, wLen = wells.length; w < wLen; w++) {
              ProjectManagement.addLabelIfDoesNotExist(wells[w].wellLabel);
            }
          }
        }

        ProjectManagement.updateNewLabelsFromOld(selectedLabels)

        $scope.labelCollection = ProjectManagement.labels;
      };
      
      $scope.updateCompounds = function() {
        selectedCompounds = ProjectManagement.getOnlySelectedCompounds();
        ProjectManagement.compounds = [];
        for (var i = 0, iLen = ProjectManagement.projects.length; i < iLen; i++) {
          var project = ProjectManagement.projects[i];
          if (project.selected == true) {
            var wells = ProjectManagement.getWells(project.title);
            for (var w = 0, wLen = wells.length; w < wLen; w++) {
              ProjectManagement.addCompoundIfDoesNotExist(wells[w]);
            }
          }
        }
        ProjectManagement.updateNewCompoundsFromOld(selectedCompounds)

        $scope.compoundCollection = ProjectManagement.compounds;
      };
      
      $scope.updateSubstrates = function() {
        selectedSubstrates = ProjectManagement.getOnlySelectedSubstrates();
        ProjectManagement.substrates = [];
        for (var i = 0, iLen = ProjectManagement.projects.length; i < iLen; i++) {
          var project = ProjectManagement.projects[i];
          if (project.selected == true) {
            var wells = ProjectManagement.getWells(project.title);
            for (var w = 0, wLen = wells.length; w < wLen; w++) {
              ProjectManagement.addSubstrateIfDoesNotExist(wells[w]);
            }
          }
        }

        ProjectManagement.updateNewSubstratesFromOld(selectedSubstrates)

        $scope.substrateCollection = ProjectManagement.substrates;
      };
